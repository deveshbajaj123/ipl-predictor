...
# Count only legal deliveries (exclude wides and no-balls)
legal = deliveries_first[
    (deliveries_first['extras_type'] != 'wides') &
    (deliveries_first['extras_type'] != 'noballs')
]
legal_balls = (
    legal.groupby('match_id')['ball']
         .count()
         .reset_index()
         .rename(columns={'ball':'balls'})
)

# 1. First-innings summary stats (include wides/noball runs in total_runs) (include wides/noball runs in total_runs)
i_summary = deliveries_first.groupby('match_id').agg(
    total_runs=('total_runs','sum'),
    wickets=('player_dismissed', lambda x: x.notna().sum())
).reset_index()
i_summary = i_summary.merge(legal_balls, on='match_id', how='left').fillna(0)

# 2. Best batter in first innings
bat_stats = deliveries_first.groupby([
...
bat_stats['half_century'] = (
    (bat_stats['bat_runs'] >= 50) & (bat_stats['bat_runs'] < 100)
).astype(int)
bat_stats = bat_stats.merge(
    matches[['id','team1']], left_on='match_id', right_on='id'
)
...
# 3. Best bowler in first innings
bowl_stats = deliveries_first.groupby([
...
bowl_stats = bowl_stats.merge(
    matches[['id','team2']], left_on='match_id', right_on='id'
)
...
# Merge all features
df = matches.merge(i_summary, how='left', left_on='id', right_on='match_id')
df = df.merge(best_bat, how='left', left_on='id', right_on='match_id')
df = df.merge(best_bowl, how='left', left_on='id', right_on='match_id')
...
df['team1_home'] = df.apply(
    lambda r: int(r['venue'] in home_venues.get(r['team1'],[])), axis=1
)

# Match stage features
df = df.sort_values(by=['season','id']).reset_index(drop=True)
...
df['season_weight'] = df['season'].apply(
    lambda x: 3 if x==current_season else (2 if x>two_years else 1)
)
...
rf_search = RandomizedSearchCV(
    ...
    param_grid_rf, n_iter=20, cv=5, scoring='accuracy', n_jobs=-1, random_state=42
)
